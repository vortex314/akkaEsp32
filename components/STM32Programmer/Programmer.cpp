...
Programmer::Programmer(Connector* connector, ActorRef& publisher) :
		_connector(connector), _publisher(publisher), _stm32(
				connector->getUART(), connector->getDigitalOut(LP_SCL),
				connector->getDigitalOut(LP_SDA)) {
}
...
Receive& Programmer::createReceive() {
	return receiveBuilder().match(MsgClass("timer1"),
			[this](Msg& msg) {
				_stm32.resetSystem();
				uint16_t id=0;
				if ( _stm32.getId(id) ) {
