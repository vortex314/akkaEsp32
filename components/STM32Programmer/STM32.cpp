...
STM32::Symbol STM32::symbols[] = { { BL_GET, "get" }, //
		{ BL_GET_ID, "getId" }, //
		{ BL_ERASE_MEMORY, "eraseMemory" }, //
		{ BL_EXTENDED_ERASE_MEMORY, "extendedEraseMemory" }, //
		{ BL_GET_VERSION, "getVersion" }, //
		{ BL_GO, "go" }, //
		{ BL_READOUT_PROTECT, "readOutProtect" }, //
		{ BL_READOUT_UNPROTECT, "readOutUnprotect" }, //
		{ BL_READ_MEMORY, "readMemory" }, //
		{ BL_WRITE_MEMORY, "writeMemory" }, //
		{ BL_WRITE_PROTECT, "writeProtect" }, //
		{ BL_WRITE_UNPROTECT, "writeUnprotect" } //
};
...
const char* STM32::findSymbol(uint8_t hex) {
	for (Symbol s : symbols) {
		if (s.cmdByte == hex) return s.cmdString;
	}
	return "unknown";
}
...
Erc STM32::writeMemory(uint32_t address, Bytes& data) {
	...
	uint8_t ADDRESS[] = { slice(address, 3), slice(address, 2),
			slice(address, 1), slice(address, 0), xorBytes(ADDRESS, 4) };
	Bytes out(0);
	Bytes noData(0);
	flush();
	Erc erc = E_OK;
	if ((erc = waitAck(out.map(GET, 2), _in, DELAY)) == E_OK) {
		if ((erc = waitAck(out.map(ADDRESS, 5), _in, DELAY)) == E_OK) {
			...
			_uart.write(((uint8_t) (data.length() - 1))
...
Erc STM32::eraseMemory(Bytes& pages) {
	...
	_uart.write(((uint8_t) (pages.length() - 1))
...
Erc STM32::extendedEraseMemory() {
	uint8_t EXTENDED_ERASE_MEMORY[] = { BL_EXTENDED_ERASE_MEMORY,
			XOR(BL_EXTENDED_ERASE_MEMORY) };
...
Erc STM32::writeProtect(Bytes& sectors) {
	...
	_uart.write(((uint8_t) (sectors.length() - 1))
