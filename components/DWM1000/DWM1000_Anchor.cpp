...
MsgClass ANCHOR_INTERRUPT(UID("ANCHOR_INTERRUPT"));
...
DWM1000_Anchor::DWM1000_Anchor(ActorRef& publisher, Spi& spi, DigitalIn& irq,
		...
		:  DWM1000(spi, irq, reset, shortAddress, longAddress), _irq(irq),_publisher(publisher)
...
Receive& DWM1000_Anchor::createReceive() {
	...
	.match(MsgClass(UID("logTimer")), [this](Msg& msg) {
	...
	.match(MsgClass(UID("blinkTimer")), [this](Msg& msg) {
