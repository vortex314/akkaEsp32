...
UltraSonic::UltraSonic(va_list args) : _uext(2) {
    //	_hcsr = new
    // HCSR04(_uext.getDigitalOut(LP_SCL),_uext.getDigitalIn(LP_SDA));
...
void UltraSonic::preStart() {
    _hcsr->init();
    _measureTimer =
        timers().startPeriodicTimer("measureTimer", Msg("measureTimer"), 100);
}

Receive& UltraSonic::createReceive() {
    return receiveBuilder().match(
        MsgClass("measureTimer"), [this](Envelope& msg) {
            ...
            if (cm < 400 && cm > 0) {
                ...
                /*		INFO("distance : %d micro-sec = %d cm ",
                   _delay, _distance);*/
            }
            _hcsr->trigger();
