	...
	public:
		...
		Erc init() {
			...
			gpio_int_type_t interruptType =
					(gpio_int_type_t) GPIO_PIN_INTR_DISABLE;
			...
			io_conf.pull_down_en = (gpio_pulldown_t) (
					_mode & DIN_PULL_DOWN ? 1 : 0);
...
#define ESP_INTR_FLAG_DEFAULT 0
			if (_fp) {
				...
				if (_fp)
					erc =
							gpio_isr_handler_add((gpio_num_t) _gpio, _fp, (void*) _object);
				if (erc)
...
class DigitalOut_ESP32: public DigitalOut {
	...
	public:
		...
		Erc init() {
			...
			esp_err_t erc =
					gpio_set_direction((gpio_num_t) _gpio, GPIO_MODE_OUTPUT);
			if (erc)
			ERROR("gpio_set_direction():%d", erc);
			gpio_config_t io_conf;
			// disable interrupt
...
struct AdcEntry {
		...
		ADC1_GPIO36_CHANNEL, ADC1 }, { 39, ADC1_GPIO39_CHANNEL, ADC1 } }; // INCOMPLETE !!
	...
	public:
		...
		Erc init() {
			...
			if (_unit == ADC1) {
				...
				//    ADC_ATTEN_DB_11);
				erc =
						adc2_config_channel_atten(ADC2_GPIO15_CHANNEL, ADC_ATTEN_DB_11);
				if (erc)
...
class UART_ESP32: public UART {
	...
	public:
		...
		Erc write(const uint8_t* data, uint32_t length) {
			if (uart_write_bytes(_uartNum, (const char*) data, length)
					== length) return E_OK;
			return EIO;
		}
		;
					...
					switch (event.type) {
						...
						case UART_PATTERN_DET: {
							...
							if (pos == -1) {
								...
								uart_read_bytes(_uartNum, dtmp, pos, 100
										/ portTICK_PERIOD_MS);
								uint8_t pat[PATTERN_CHR_NUM + 1];
								memset(pat, 0, sizeof(pat));
								uart_read_bytes(_uartNum, pat, PATTERN_CHR_NUM, 100
										/ portTICK_PERIOD_MS);
...
Spi& Connector::getSPI() {
	_spi =
			new SPI_ESP32(toPin(LP_MISO), toPin(LP_MOSI), toPin(LP_SCK), toPin(LP_CS));
	lockPin(LP_MISO);
	lockPin(LP_MOSI);
	lockPin(LP_SCK);
	lockPin(LP_CS);
